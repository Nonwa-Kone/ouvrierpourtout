...
import { helperInitial } from '../../../assets/utils';
...
export const Avatar = ({
  ...
  const fontSizeMax = 50;
  const maxSize = 120;

  const proportion = fontSizeMax / maxSize;

  const fontSize = proportion * size;
  return (
    <div
      className='usersItemsLayout--avatar'
      style={{
        width: size,
        height: size,
        position: 'relative',
    ...
    >
      <h2 className='usersItemsLayout--avatarText' style={{ fontSize }}>
        {helperInitial(fullname?.split(' ')[0], fullname?.split(' ')[1])}
      </h2>
      {Icon && (
        <div
          className='usersItemsLayout--avatarIcon'
          style={{
            position: 'absolute',
            // top: '0',
            // right: '0',
...
export const TableHeader = ({
  ...
  columns?: { label: string; flex?: number }[] | null;
    ...
    <div className='tableLayout' style={style}>
      ...
      >
        ...
        {columns?.map((column, index) => (
          // const Icon = columns === 'cash In' ? TransferIcon : CollectIcon;
          <>
            <div
              className='tableLayout--cell'
              style={{
...
const CustomBasicPaginator: React.FC<CustomBasicPaginatorProps> = ({
  ...
  const updateVisiblePages = (page: number) => {
    ...
    const pageNumbers = Array.from(
      { length: endPage - startPage + 1 } as ArrayLike<number>,
      (_, i) => startPage + i
    );
