...
const authOuvrier = (req, res, next) => {
  try {
    // Vérification de l'en-tête d'autorisation
    const authHeader = req.headers.authorization;
    console.log('🚀 ~ authOuvrier ~ authHeader:', authHeader);
    ...
    const decoded = jwt.verify(token, process.env.SECRET_TOKEN_PARTNER_LOGIN);
    console.log('🚀 ~ authOuvrier ~ decoded:', decoded);
    // Assigner les informations décodées à la requête
    req.ouvrier = decoded;
    // Poursuivre vers le middleware suivant
    next();
  } catch (error) {
    console.error('🚀 ~ authOuvrier ~ error:', error);
